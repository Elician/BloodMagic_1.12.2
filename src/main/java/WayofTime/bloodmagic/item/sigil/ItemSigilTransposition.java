    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos blockPos, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (!world.isRemote) {
            if (BloodMagicAPI.INSTANCE.getBlacklist().getTransposition().contains(state))
                return EnumActionResult.FAIL;
            //TODO: Goodbird code start
            GDClaim gclaim= GriefDefenderPlugin.getInstance().dataStore.getClaimAt(new Location((Extent) player.world,blockPos.getX(),blockPos.getY(),blockPos.getZ()));
            boolean canUseInRg=gclaim.isUserTrusted(player.getPersistentID(),TrustTypes.BUILDER) || gclaim.isWilderness();
            if(!canUseInRg)
                return EnumActionResult.FAIL;
            //TODO: Goodbird code end
